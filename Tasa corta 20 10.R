...
modtasa=setModelCC(drift="a*(b-x)", diffusion= "sigma", solve.variable="x")
estimal=setYuima(data=setData(db_entrena, delta=1/360), model)

param.init<-list(a=a0, b=b0, sigma=s0)
low.par<-list(a=-10, b=.01, sigma=.00001)
upp.var<-list(a=10, b=1, sigma=1)
mle1<-qmle(estimal,start=param.init,method="L-BFGS-B", lower=low.par, upper=upp.var)
...
for(k in (1:tray)){
r=simulate(modtasa, xinit=db_prueba[1], sampling = samp, true.parameter = list(a=mle1@coef[2], b=mle1@coef[3], sigma=mle1@coef[1]))
...
plot(db_prueba, type="l", col="red", ylim=c(min(quat_base), max(quat_base)))
q_90 <- rowQuantiles(r_vect, probs=.90)
lines=(q_90)
